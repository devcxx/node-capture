...
{
    return XDG_SESSION_TYPE == "wayland" || WAYLAND_DISPLAY.contains("wayland", Qt::CaseInsensitive);
}

DesktopInfo::WM DesktopInfo::windowManager()
{
    DesktopInfo::WM res = DesktopInfo::OTHER;
    if (XDG_CURRENT_DESKTOP.contains("GNOME", Qt::CaseInsensitive) || !GNOME_DESKTOP_SESSION_ID.isEmpty()) {
